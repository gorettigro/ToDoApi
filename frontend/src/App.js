...
const App = () => {
	...
	const addTodo = async todo => {
		await axios.post(`http://localhost:3800/`, {
	...
	const fetchTodos = async () => {
		const res = await axios.get('http://localhost:3800/');
	...
	const editTodo = async (id, newContent) => {
		await axios.patch(`http://localhost:3800/`, {
			content: newContent,
		});
	...
	const deleteTodo = async id => {
		await axios.delete(`http://localhost:3800/`);
